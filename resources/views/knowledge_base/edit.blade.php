...
<div class="max-w-3xl mx-auto">
    <div class="mb-8 flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 mt-1 text-sm">Update the knowledge base guide.</p>
        </div>
        <a href="{{ route('knowledge-base.show', $article->id) }}" class="inline-flex items-center text-gray-500 hover:text-gray-900 transition-colors">
        ...
        <div class="p-8">
            <form action="{{ route('knowledge-base.update', $article->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $article->title) }}" required placeholder="E.g. How to reset your password"
                    ...
                    <div class="relative">
                        <select name="category" id="category"
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->name }}" {{ old('category', $article->category) == $category->name ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="10" required placeholder="Write your article content here..."
                              class="block w-full px-4 py-3 border border-gray-200 rounded-xl bg-gray-50/50 text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all shadow-inner sm:text-sm resize-y">{{ old('content', $article->content) }}</textarea>
                    @error('content')
                        <p class="text-red-600 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center mb-4">
                    <input id="is_published" name="is_published" type="checkbox" value="1" {{ old('is_published', $article->is_published) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-700 border-gray-300 rounded focus:ring-blue-500 focus:ring-2">
                ...
                <div class="pt-6 border-t border-gray-200/50 flex justify-end gap-4">
                    <a href="{{ route('knowledge-base.show', $article->id) }}" class="px-6 py-2.5 border border-gray-300 rounded-xl shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-100 focus:outline-none transition-colors">
                        Cancel
                    </a>
