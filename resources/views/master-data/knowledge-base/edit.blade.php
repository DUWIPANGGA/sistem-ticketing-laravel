...
<div class="max-w-4xl mx-auto">
    <div class="mb-8 flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 mt-1 text-sm">Update the knowledge base article.</p>
        </div>
    ...
    <div class="bg-white border border-gray-200/50 rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="p-8">
            <form action="{{ route('master-data.knowledge-base.update', $article->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="12" required
                              class="block w-full px-4 py-3 border border-gray-200 rounded-xl bg-gray-50/50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all shadow-inner sm:text-sm resize-y">{{ old('content', $article->content) }}</textarea>
                    @error('content')
                        <p class="text-red-600 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
