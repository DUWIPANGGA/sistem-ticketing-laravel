        ...
        <table class="min-w-full divide-y divide-gray-700/50">
            <thead class="bg-gray-50/50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">SLA Hours</th>
                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Sort Order</th>
                ...
                @forelse ($priorities as $priority)
                    <tr class="hover:bg-gray-100/25 transition-colors group">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <code class="bg-gray-100 px-2 py-0.5 rounded text-xs">{{ $priority->value }}</code>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
                            <a href="{{ route('master-data.priorities.edit', $priority->id) }}" class="text-indigo-600 hover:text-indigo-300 transition-colors" title="Edit">
                                <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>
                            <form action="{{ route('master-data.priorities.destroy', $priority->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this priority?');">
                                @csrf
                                @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                <h3 class="text-lg font-medium text-gray-900 mb-1">No priorities found</h3>
                                <p class="text-gray-500 text-sm">Get started by creating a new priority.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
