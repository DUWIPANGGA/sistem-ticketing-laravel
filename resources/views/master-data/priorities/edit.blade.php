...
<div class="max-w-4xl mx-auto">
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Edit Priority</h1>
            <p class="text-gray-500 mt-1 text-sm">Update the priority level details.</p>
        </div>
    ...
    <div class="bg-white border border-gray-200/50 rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="p-8">
            <form action="{{ route('master-data.priorities.update', $priority->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="value" id="value" value="{{ old('value', $priority->value) }}" required
                           class="block w-full px-4 py-3 border border-gray-200 rounded-xl bg-gray-50/50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all shadow-inner sm:text-sm">
                    <p class="text-gray-500 text-xs mt-1">Internal key used in the database. Changing this may affect existing tickets.</p>
                    @error('value')
                        <p class="text-red-600 text-xs mt-1.5 flex items-center"><svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="sla_hours" id="sla_hours" value="{{ old('sla_hours', $priority->sla_hours) }}" required min="1"
                               class="block w-full px-4 py-3 border border-gray-200 rounded-xl bg-gray-50/50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all shadow-inner sm:text-sm">
                        @error('sla_hours')
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $priority->sort_order) }}" required min="0"
                               class="block w-full px-4 py-3 border border-gray-200 rounded-xl bg-gray-50/50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all shadow-inner sm:text-sm">
                        @error('sort_order')
                            <p class="text-red-600 text-xs mt-1.5">{{ $message }}</p>
                        @enderror
